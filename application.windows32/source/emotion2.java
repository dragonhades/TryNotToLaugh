...
public class emotion2 extends PApplet {
...
public class Instr_button implements Button
{
  ...
  public float x = width*(4/5.0f)+5+slider;
  public float y = height*(4/7.0f)-60;
  
  public void draw() {
    x = width*(4/5.0f)+5+slider;
    noFill();
    strokeWeight(10);
    stroke(255);
...
public class Start_button implements Button
{
  ...
  public float btheight = 400;
  public float x = width*(4/5.0f)+5+slider;
  public float y = height*(1/7.0f)-60;
  public void draw() {
    x = width*(4/5.0f)+5+slider;
    noFill();
    strokeWeight(10);
...
public class GameStart_button implements Button
{
  ...
  public float x = width*(4/5.0f)+5+slider;
  public float y = height*(6/7.0f)-50;
  public void draw() {
    x = width*(4/5.0f)+5+slider;
    noFill();
    strokeWeight(12);
  ...
  public boolean mouseOver() {
    boolean b = mouseX >= x && mouseX <= x + btwidth*2.3f &&
      mouseY >= y && mouseY <= y + btheight*1.2f;
    if (b) {
      noFill();
      strokeWeight(8);
      stroke(255);
      rect(x+50, y-20, btwidth*1.3f, btheight+60);
    }
    return b;
  }
}
...
public class Back_button implements Button
{
  ...
  public float btwidth = 100;
  public float btheight = 100;
  public float x = width*(4/5.0f)-300+slider;
  public float y = height*(6/7.0f)-50;
  public void draw() {
    x = width*(4/5.0f)-300+slider;
    noFill();
    strokeWeight(12);
    stroke(255);
    rect(x, y, btwidth-20, btheight-20);
...
public class Healthbar
{
  ...
  public void receive_damage(float dmg) {
    if(dmg < 0.05f) return;
    if(health != 0) {
...
public void setup() {
  ...
  scale = width*2 / PApplet.parseFloat(640);
...
public void draw() {
  ...
  case READY:
    slider = 0;
    if(cam == null){
      cam = new Capture(this, PApplet.parseInt(640)/2, PApplet.parseInt(480)/2);
    ...
    if (cam.available() == true) {
      ...
      if (cam.available() == true) {
        ...
        try {
          if (frames==6) {
            if(faces.length>0){
              PImage crop = get(PApplet.parseInt(faces[0].x*scale),PApplet.parseInt(faces[0].y*scale-240),
                  PApplet.parseInt(faces[0].width*scale),PApplet.parseInt(faces[0].height*scale));
              FloatDict emotions = emo_recog.recognizeFromCamera(crop);
              float happiness = emotions.get("happiness");
              api_debug_counter ++;
              println("\n[DEBUG]------INDEX: "+api_debug_counter+"-----"+millis()/1000.0f+"s------Happiness: "+happiness+"------------");
              hb.receive_damage(happiness);
            }
          }
          if(frames == 11){
            if(bt.mode == 2 && faces.length>1){
              PImage crop = get(PApplet.parseInt(faces[1].x*scale),PApplet.parseInt(faces[1].y*scale-240),
                  PApplet.parseInt(faces[1].width*scale),PApplet.parseInt(faces[1].height*scale));
              FloatDict emotions = emo_recog.recognizeFromCamera(crop);
              float happiness = emotions.get("happiness");
              api_debug_counter ++;
              println("\n[DEBUG]------INDEX: "+api_debug_counter+"-----"+millis()/1000.0f+"s------Happiness: "+happiness+"------------");
...
public void drawHealthBar(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "emotion2" };
    if (passedArgs != null) {
